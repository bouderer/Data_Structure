...
class BinarySearchTree{
    ...
    private:
        ...
        int count(BinTree<T>* bt)
        {
			if(bt==NULL) return 0;
			int a=1+count(bt->getLChild())+count(bt->getRChild());
			return a;
		}
    ...
    public:
		...
		int all(BinTree<T>* bt,int mod)
		{
			if(bt==NULL) return 1;
			int L=count(bt->getLChild());
			        	int R=count(bt->getRChild());
			        	int a=L+R;
			        	int n=(comb(R,a,mod)*all(bt->getLChild(),mod)%mod)*all(bt->getRChild(),mod)%mod;
			        	return n;
		}
